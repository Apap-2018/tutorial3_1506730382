...
public class PilotController {
	...
	@RequestMapping(value = {"/pilot/update/license-number/","/pilot/update/license-number/{licenseNumber}/fly-hour/{flyHours}"})
	public String updateFlyHours(@PathVariable Optional<String> licenseNumber,
						   		 @PathVariable int flyHours,
						   		 Model model) {
		if (licenseNumber.isPresent()) {
			if (pilotService.searchByLicenseNumber(licenseNumber.get())) {
				pilotService.updateFlyHours(licenseNumber.get(),flyHours);
				
				return "flyhour-updated";
			}
			else return "not-found";
		}
		else return "not-found";	
